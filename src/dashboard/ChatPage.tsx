...
// Convex wraps action errors in a generic "Server Error Called by client".
// This digs out the real message so the user sees the actual cause.
...
export default function ChatPage() {
  ...
  const handleNewConversation = async () => {
    ...
    try {
      ...
      setErrorMsg(readError(error, "Failed to create conversation"));
    }
  };
  ...
  const handleSendMessage = async () => {
    ...
    if (!convId) {
      try {
        ...
        setErrorMsg(readError(error, "Failed to start conversation"));
        return;
      }
    }
    ...
    try {
      ...
      setErrorMsg(readError(error, "Failed to send message"));
  ...
  const handleDeleteConversation = async (convId: Id<"conversations">) => {
    ...
    try {
      ...
      setErrorMsg(readError(error, "Failed to delete conversation"));
    }
  };
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          {conversations.length === 0 ? (
            ...
            conversations.map((conv) => (
              <div
                key={conv._id}
                className={`w-full rounded-xl transition-colors overflow-hidden group ${
